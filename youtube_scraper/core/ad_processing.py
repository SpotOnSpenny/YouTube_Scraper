...
def process_data(response, index, clicks, search_term, profile, date):
    # ----- Colored Messages -----
    no_ads = colored(
        "No ads found on video #{}, processing next video".format(clicks), "magenta"
    )
    ad_added = colored("An Ad has been found and added to the index!", "green")
    already_downloaded = colored(
        "A duplicate ad was found, no need to download!", "magenta"
    )
    ...
    duplicates = 0
    ads = list(
        find_values(response, "instreamVideoAdRenderer")
    )  # determine if an ad exists on the video
    processed = 0
    if ads == []:  # when there are no ads, move on
        ...
        for ad in ads:
            try:
                ...
                with open(
                    "no externalVideoID.json", "w"
                ) as outfile:  # save json response for troubleshooting
                    json.dump(response, outfile)
                continue  # next iteration
            if (
                check_for_duplicate(ad_id) == False
            ):  # if ad hasn't been downloaded yet, download it
            ...
            print(ad_added)
        index.to_csv(
            path_or_buf="./youtube_scraper/downloaded_ads/ad_index.csv", index=False
        )  # save CSV incase of error
        return processed, index, ad_present, new, duplicates
...
def download_ad(ad_id):
    # ----- Colored Messages -----
    download_success = colored(
        "Ad downloaded successfully and can be found in the downloaded ads folder.",
        "green",
    )
    download_failed = colored(
        "Ad could not be downloaded, set property to false to download later", "red"
    )
    ...
    ad = YouTube(ad_url)
    try:
        ad.streams.get_highest_resolution().download(
            output_path=ad_path, filename="{}.mp4".format(ad_id), max_retries=3
        )
